...
center = np.array((10.25, 0.76))  # center of work area
...
for k, v in key.items():
    if m := modules.get(k):
        ...
        if k.startswith("L"):
            ...
            # magic number is empirically determined.
            # I don't understand why it is not possible to move it by
            # x units

            pos = move_angle(keypos, -5.625* unit * 1e6, (90 - deg) * 2 * np.pi / 360)
...
for m, v in modules.items():
    ...
    if m.lower().startswith("u2"):
        ...
        pos = list(top.GetCenter())
        # pos = move_angle(pos, 1.1 * scale * unit, 0)
        pos = move_angle(pos, 1.1 * scale * unit, 0)

        # pos = move_angle(pos, 0.9 * scale * unit, 0)
        top = top.GetBoundingBox().GetHeight()
        h = v.GetBoundingBox().GetHeight()
        target = pos[1]
        ...
        if m.lower().endswith("right1"):
            ...
            pos = move_angle(pos, 2 / 16 * w * unit, theta=0)
            print(pos)
...
for m, v in modules.items():
    ...
    if m == "EC11":
        ...
        pos = np.array([pos.x - width / 4, height * 0.4], dtype = float)
        print(pos)
...
fig.show()
 ...
 #+RESULTS:
 :RESULTS:
 : [3.30285556e+08 5.32982838e+07]
 : [3.36856668e+08 2.26228548e+07]
 : [3.20395138e+08 2.26228548e+07]
 : <pcbnew.MODULE; proxy of <Swig Object of type 'MODULE *' at 0x7fca468338a0> >
 [[file:./.ob-jupyter/e568b79430a8f2ed531cabc1ac22dac5e06f22fd.png]]
 :END:
...
pcb.Save(os.path.join(base, board_fp))
 ...
 : (120, 2)
 : True
 [[file:./.ob-jupyter/668fdaafe1699269bd24231e56b1a7f7bad6921b.png]]
 :END:
